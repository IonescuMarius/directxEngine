...
#ifndef _DXMeshAnimation_H_
#define _DXMeshAnimation_H_
...
class dxMeshAnimation
{
	...
	public:
		...
		void Render() const;
		void ReleaseMesh();
		bool ComputeBoundingBox(D3DXMATRIX *matWorld);
		...
		HRESULT             LoadActorFromX              ( LPCTSTR FileName, ULONG Options, LPDIRECT3DDEVICE9 pD3DDevice, bool ApplyCustomSets = true, void * pCallbackData = NULL );
		...
		// Utility Functions
		bool                IsLoaded                    ( ) const { return (m_frameRoot != NULL); }
	...
	protected:
		...
		//-------------------------------------------------------------------------
		LPDIRECT3DDEVICE9			dx_d3dDevice;
...
{
public:
	CAllocateHierarchy( dxMeshAnimation * pActor ) : m_pActor(pActor){}
    ...
    //-------------------------------------------------------------------------
    dxMeshAnimation  * m_pActor;         // Actor we are allocating for
...
public:
    ...
    HRESULT         ApplyAction             ( dxMeshAnimation * pActor );
